...
from app.models.tango import TangoChouModel, TangoCardModel
from app.dependencies import ValidateTokenDep, get_firestore
from app.internal.tango_actions import (
    get_all_tango_chou,
    get_tango_chou,
    add_word_to_tango_chou,
)
...
@tango_router.get("/")
def all_tango_chou(
    validate_token: ValidateTokenDep, firestore=Depends(get_firestore)
) -> list[TangoChouModel]:
...
def create_tango_chou(
    ...
    print("Token: " + str(type(validate_token)))

    return JSONResponse(
        content={
            "tango_chou_id": create_tango_chou(tango_chou_name, uid, firestore),
        }
    )
